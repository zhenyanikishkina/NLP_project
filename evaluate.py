...
from models import HFmodel, miniLMSPECTER, miniLMSPECTER2
    ...
    def evaluate(
        ...
        for dataset in datasets:
            ...
            for model_path, pooling in models:
                ...
                ranking = self.rank(
                    retriever, corpus, queries, skip_first=False
                )
                rankings.append(ranking)

                key2history_doc = dict()
                for k in ranking.keys():
                    cur_corpus = []
                    cur_corpus_keys = []
                    for cur_k in ranking[k].keys():
                        cur_corpus.append(corpus[cur_k]['title'])
                        cur_corpus_keys.append(cur_k)
                    tokenized_corpus = [doc.split(" ") for doc in cur_corpus]
                    bm25 = BM25Okapi(tokenized_corpus)

                    cur_query = ""
                    for imp in qrels[k].keys():
                        if qrels[k][imp] and imp in corpus:
                            cur_query = corpus[imp]['title']
                            break
                    tokenized_query = cur_query.split(" ")

                    doc_title = cur_corpus[np.argmax(bm25.get_scores(tokenized_query))]
                    queries[k] += ("@@@" + doc_title)

                new_model = miniLMSPECTER2(
                            model_path=model_path, pooling=pooling, sep=sep
                        )

                retriever_iterative = self.create_index(new_model, corpus, batch_size, dataset)
                ranking_iterative = self.rank(
                    retriever_iterative, corpus, queries, skip_first=False
                )
                rankings_iterative.append(ranking_iterative)
            ...
            if fusion == "rrf":
                ...
                results = rankings[0]
                results_iterative = rankings_iterative[0]
            else:
